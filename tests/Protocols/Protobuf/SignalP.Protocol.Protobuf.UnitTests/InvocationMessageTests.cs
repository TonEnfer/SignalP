...
public class InvocationMessageTests
{
    ...
    public void Protocol_Should_Handle_Invocation_Message_Without_Parameters_And_Headers(string target)
    {
        ...
        var completionMessage = new InvocationMessage(target, []);
        ...
        resultCompletionMessage.Should().NotBeNull().And.BeOfType<InvocationMessage>().Subject.Should().BeEquivalentTo(completionMessage);
    ...
    public void Protocol_Should_Handle_Invocation_Message_Without_Parameters_And_Headers_With_InvocationId(string invocationId)
    {
        var binder = new Mock<IInvocationBinder>(MockBehavior.Strict);
        binder.Setup(x => x.GetParameterTypes("target")).Returns([]);
        var protobufHubProtocol = new ProtobufHubProtocol();
        ...
        var completionMessage = new InvocationMessage(invocationId, "target", []);
    ...
    public void Protocol_Should_Handle_Invocation_Message_With_StreamIds(params string[] streamIds)
    {
        var binder = new Mock<IInvocationBinder>(MockBehavior.Strict);
        ...
        var completionMessage = new InvocationMessage("invId", "target", [], streamIds)
        {
    ...
    public void Protocol_Should_Handle_Invocation_Message_With_Parameters_And_Headers_And_InvocationId(object[] data)
    {
        var binder = new Mock<IInvocationBinder>(MockBehavior.Strict);
        ...
        var completionMessage = new InvocationMessage("invId", "target", data)
        {
            Headers = new Dictionary<string, string>
            {
                { "%#^^D\t\b\n\r", "%#^^D\t\b\n\r321" },
